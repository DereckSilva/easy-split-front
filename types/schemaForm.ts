...
export const userCreateSchema = yup.object({  
  ...
  birthdate: yup.date().typeError("Data Inválida").required('Data de nascimento é obrigatória').min(new Date()).default(new Date()),
  phoneNumber: yup.string().matches(/^(\(?\d{2}\)?\s?)?(9\d{4})-?\d{4}$/, 'Número de telefone inválido').required('Número de telefone é obrigatório'),
});